...
class boxberryShippingApiManager
{
    ...
    const METHOD_NDD_SETTINGS = 'CompanyApiSettings';
    ...
    /**
     * https://help.boxberry.ru/spaces/TA/pages/120913927/CompanyApiSettings.+%D0%9F%D0%BE%D0%BB%D1%83%D1%87%D0%B5%D0%BD%D0%B8%D0%B5+%D1%81%D1%82%D0%B0%D1%82%D1%83%D1%81%D0%B0+%D0%BF%D0%B5%D1%80%D0%B5%D1%85%D0%BE%D0%B4%D0%B0+%D0%BA%D0%BB%D0%B8%D0%B5%D0%BD%D1%82%D0%B0+%D0%B2+NDD
     * @return bool
     * @throws waException
     */
    public function isNdd()
    {
        $result = false;
        if (!$this->bxb->getSettings('yaDelivery')) {
            $response = $this->sendRequest(['method' => self::METHOD_NDD_SETTINGS]);
            $result = ifset($response, 'yaDelivery', false);
            if ($result) {
                $this->bxb->saveSettings(['yaDelivery' => 1]);
            } else {
                // трактуем как клиент не перешел на NDD (Я.доставку), см. докум.
                $this->errors = null;
                $this->bxb->saveSettings(['yaDelivery' => 0]);
            }
        }
