...
class boxberryShipping extends waShipping
{
    ...
    {
        $bxb_settings = $this->getSettings();

        if ((int) ifset($bxb_settings, 'yaDelivery', 0) === 1 && !empty($settings['targetstart'])) {
            $bxb_settings['yaDelivery'] = 2;
        }

        foreach ($settings as $key => $value) {
            $bxb_settings[$key] = $value;
        }

        parent::saveSettings($bxb_settings);
    }
}
