...
(function ($) {
    ...
    methods = {
        ...
        send: function (target, toLowerCase) {
            if (toLowerCase) {
                return $(target).val($.transliterate($(this).val()).toLowerCase());
            } else {
                return $(target).val($.transliterate($(this).val()));
            }
        },
        watch: function (target, unwatchOnChange, toLowerCase) {
            return this.each(function () {
                var data, that;
                ...
                $(target).on('change.translit input.translit keyup.translit propertychange.translit', function () {
                    $(this).translit('send', that, toLowerCase);
                });
                if (unwatchOnChange) {
