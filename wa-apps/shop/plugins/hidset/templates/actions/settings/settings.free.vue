...
<div class="tab-content">
            ...
            <div class="hidset-row" v-for="(data, set) in sets">
                <template v-if="data.type!=='icons'">
                    ...
                    <template v-if="data.type === 'int'">
                        <div class="hidset-cell"><input type="number" class="shorter" v-model="settings[set]" min="0">
                        </div>
                    </template>
                        ...
                        <div class="hidset-cell">
                            <table class="small" style="margin-bottom: 0px !important;">
                                <tr v-for="(el, idx) in data.options">
                                    ...
                                    <td>
                                        <input type="number" class="shortest" v-model="settings[set][el.name]">
                                    </td>
                                </tr>
                            </table>
                        </div>
    ...
    <span class="alert info" style="color: black">
        ...
        <span v-if="pluginSets.length>0"><input id="hidset-default-all" type="radio" name="defaultValue" v-model="defaultValues" value="all"> <label
                ...
                <action-button @click="restoreDefaultSettings()" icon="fas fa-arrows-alt-h" title="[`Восстановить`]" :run="runActions"></action-button>
                <action-button @click="defaultValues=false" icon="fas fa-undo-alt" title="[`Отмена`]" action="tmp" :run="runActions"></action-button>
            </template>
            <action-button @click="saveSettings('base')" v-if="defaultValues===false" title="[`Сохранить`]" action="saveSettings" icon="fas fa-save" :run="runActions"></action-button>
            <action-button @click="defaultValues='shop_hidset'" v-if="defaultValues===false" title="[`Восстановить значения по-умолчанию...`]" icon="fas fa-arrows-alt-h" action="tmp" :run="runActions" style="margin-left: 70px;" bClass="outlined"></action-button>
        </div>
    </template>
