...
<template v-if="dimEdit===false">
    <div>
        <select v-model="currentDim" v-if="newDim===false">
        ...
        <template v-else>
            <action-button v-if="premium===true" @click="newDim={'id':'', 'name': '', 'base_unit': ''}" icon="fas fa-plus" title="[`Добавить новую меру`]" :run="runActions" style="float: none !important; margin-left: 150px;" bClass="smaller"></action-button>
        </template>
    </div>
</template>
<template v-if="currentDim !== false && newDim===false">
    <template v-if="dimEdit!==false">
        ...
        <table id="hidset-dimensions" class="mono" style="width: unset;">
        ...
        <div v-if="newUnit!==false">
            <table id="hidset-dimensions" class="mono" style="width: unset !important;">
                <tr>
    ...
    <div class="hidset-footer">
        <template v-if="dimEdit===false">
            <template v-if="premium===true">
                <action-button @click="onEditDim(currentDim)" title="[`Изменить`]" icon="far fa-edit" :run="false" bClass="smaller"></action-button>
                <action-button @click="restoreDefaultDim()" icon="fas fa-sync" title="Восстановить значения по умолчанию" :run="runActions" style="margin-left: 35px;" bClass="smaller"></action-button>
                <action-button @click="delDim()" icon="fas fa-trash-alt" title="[`Удалить`]" :run="runActions" bClass="smaller red outlined"></action-button>
            </template>
        </template>
        <template v-else-if="newUnit===false">
            <action-button @click="saveDim()" :disabled="!dimEdit.units.hasOwnProperty(dimEdit.base_unit)" title="Сохранить" icon="fas fa-save" :run="runActions" bClass="smaller"></action-button>
            <action-button @click="currentDim='length';dimEdit=false;" title="[`Отмена`]" icon="fas fa-undo-alt" action="tmp" :run="runActions" bClass="smaller outlined"></action-button>
        </template>
    </div>
</template>
</div>
