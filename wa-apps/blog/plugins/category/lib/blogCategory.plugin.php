...
class blogCategoryPlugin extends blogPlugin
{
    ...
    public function postUpdate($post)
    {
        ...
        if (isset($post['plugin']) && isset($post['plugin'][$this->id])) {
            ...
            $category_model->changePost($post_id, $categories);

            $category_data_model = new blogCategoryModel();
            $all_categories = $category_data_model->getById($categories);

            $category_names = [];
            foreach ($categories as $category_id) {
                if (isset($all_categories[$category_id])) {
                    $category_names[$category_id] = ['name' => $all_categories[$category_id]['name'], 'url' => $all_categories[$category_id]['url']];
                }
            }

            $post_model = new blogPostModel();
            $post_model->updateById($post_id, [
                'categories' => json_encode($category_names, JSON_UNESCAPED_UNICODE) // Сохраняем с поддержкой Unicode
            ]);
        }
    }
...
HTML;
    ...
    public static function getPostCategories($post_id)
    {
        ...
        if (!empty($post['categories'])) {
            $post['categories'] = json_decode($post['categories'], true); // Преобразуем JSON в ассоциативный массив
